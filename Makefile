# This Makefile is for the CGL::Global extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/CGL/Global.pm]
...
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/CGL/Global.pm]
#     clean => { FILES=>q[CGL-Global-*] }
...
INST_LIBDIR      = $(INST_LIB)/CGL
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/CGL
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''JF Abril <jabril@ub.edu>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: CGL-Global' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "JF Abril <jabril@ub.edu>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "CGL-Global",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="CGL-Global" VERSION="1">' > CGL-Global.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>JF Abril &lt;jabril@ub.edu&gt;</AUTHOR>' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.18" />' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> CGL-Global.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> CGL-Global.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/CGL/Global.pm' 'blib/lib/CGL/Global.pm' 
